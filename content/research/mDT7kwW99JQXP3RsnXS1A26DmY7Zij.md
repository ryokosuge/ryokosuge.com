---
date: 2025-12-08T02:06:07+09:00
draft: false
...
具体的には、お問い合わせを最初にAI Agentが受け取り、カテゴライズして専門のAgentにパスし、対応できない場合はSlackでCSチームにエスカレーションする仕組みを作りたい。
...
1. Orchestrator Agent（AgentCore Gatewayで制御）がお問い合わせを受け取る
2. 内容をカテゴライズ
3. カテゴリごとの専門Agent（請求担当、技術サポート、一般問い合わせ等）にパス
4. 対応不可の場合はSlack通知でCSチームにエスカレーション
...
Amazon Bedrock AgentCoreを使えば、マルチエージェント構成でお問い合わせ対応システムを構築できることが明確になった。特にAgentCore Memory、Gateway、Policyなど本番運用に必要な機能が揃っており、公式ドキュメントにカスタマーサポート向けのシナリオも用意されている点が心強い。

### わからなかったこと

Slack通知の具体的な実装方法や、AgentCoreの各機能（Memory、Gateway、Policy等）の詳しい使い方がまだ不透明。また東京リージョンでの提供がまだのため、レイテンシーの観点も今後検討が必要。
