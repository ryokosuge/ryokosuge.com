...
### Development
```bash
# Start Hugo development server (via devcontainer)
make server
# This builds the devcontainer image and starts Hugo server at http://localhost:1313

# Stop the devcontainer
make stop

# Start Hugo server locally (without devcontainer, requires Hugo installed)
make server-local
```
...
- `/save-conversation` - Manually save Claude conversations to blog (also runs automatically via hooks)
...
- `bin/`: Automation scripts
  - `auto-save-conversation.sh`: Hook script for automatic conversation saving

## Automated Workflows (Hooks)

This project uses Claude Code's hook system to automatically save conversations without manual intervention.

### Active Hooks

**Auto-Save Conversation Hook**
- **Triggers**: SessionEnd, PreCompact
- **Script**: `bin/auto-save-conversation.sh`
- **Behavior**: Automatically executes `/save-conversation` command when:
  - A Claude Code session ends
  - Context is about to be compacted (due to conversation length)
- **Features**:
  - Infinite loop prevention using `CLAUDE_HOOK_RUNNING` flag
  - Runs in background to avoid blocking
  - No user interaction required
  - All conversations are automatically archived to `content/conversations/`
...
1. When a trigger event occurs (SessionEnd or PreCompact), Claude Code executes `bin/auto-save-conversation.sh`
2. The script checks if a hook is already running to prevent infinite loops
3. If not running, it sets the `CLAUDE_HOOK_RUNNING` flag and executes `/save-conversation`
4. The `/save-conversation` command analyzes the conversation and creates a new markdown file in `content/conversations/`
5. Metadata (title, description, tags) is automatically generated by AI analysis

### Benefits

- **Zero-effort archiving**: No need to manually save interesting conversations
- **Complete history**: All conversations are preserved automatically
- **Searchable knowledge base**: Conversations become searchable blog posts
- **Learning journal**: Track your development journey over time
...
**自動保存**: SessionEnd/PreCompactフックにより、すべての会話が自動的に保存されます（手動実行不要）
**手動保存**: `/save-conversation` コマンドで任意のタイミングで保存も可能

### 自動保存の仕組み
- セッション終了時またはコンテキスト圧縮前に自動実行
- `bin/auto-save-conversation.sh` が `/save-conversation` コマンドを呼び出し
- ユーザー操作不要で完全自動
